...
class CrosswordCreator():
        ...
        #test conflicts
            for n in self.crossword.neighbors(v):
                if n not in assignment.keys():
                    continue
                (x,y) = self.crossword.overlaps[v,n]
                if assignment[v][x] != assignment[n][y]:
                    return False
    ...
    def order_domain_values(self, var, assignment):
        ...
        sd = dict()
        for wv in self.domains[var]:
            count = 0
            for n in neighbours:
                (x,y) = self.crossword.overlaps[var,n]
                for wn in self.domains[n]:
                    if wv[x] != wn[y]:
                        count += 1
            sd[wv]=count
    ...
    def select_unassigned_variable(self, assignment):
        ...
        m = float('inf')
        for i in l:
            m = min( len(self.domains[i]), m)
        
        for i in l:
            if len(self.domains[i]) != m:
                l.remove(i)
        if len(l) == 1:
            return l[0]

        #max degree
        l.sort(key = lambda i: len(self.crossword.neighbors(i)) )
